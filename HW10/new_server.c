...
char * clientComm(int clntSockfd,int * senderBuffSize_addr, int * optlen_addr){
    ...
    int numBytes = 0;
    // The reason of the buffer overflow is because of the space allocated for input string is too small.
    // By changing the way of allocating space, the vulnerability could be fixed.
    // The original way of allocating space was depended on default value, the new way of allocating space is allocating
